...
export function getObjectByUUID(uuid) {
    return objects.find(object => object.uuid === uuid) || null;
}
...
export function syncObjects(scene) {
    ...
    scene.traverse(object => {
        ...
        if (isEditorObject(object) && (object.isMesh || object.isGroup || object.userData?.selectable)) {
            if (!objects.includes(object)) objects.push(object);
        }
    });
