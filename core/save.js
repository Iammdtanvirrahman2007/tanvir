...
export function serializeScene(scene, options = {}) {
    ...
    const voxel = typeof window !== "undefined" ? window.__modelForgeVoxelEditor?.getGrid?.()?.serialize?.() ?? null : null;
    return createRKPAsset({ id: options.assetId || getProjectAssetId(), name: options.name || getProjectName(), type: options.assetType || "model", category: options.category || "model", tags: options.tags || [], author: options.author || "", creator: options.creator || "", objects, scene: { objects }, materials: collectMaterials(objects), dimensions: calculateDimensions(scene), description: options.description || "", voxel });
}
...
export function saveScene(scene, options = {}) {
    ...
    if (format === "rkp") {
        ...
        if (!validation.valid) { window.dispatchEvent(new CustomEvent("editor:validation-failed", { detail: validation })); return false; }
    }
...
export function chooseSaveFormat(scene) {
    ...
    if (existing) { const input = existing.querySelector("#saveFilename"); input?.focus(); input?.select(); return existing; }
    installSaveDialogStyles();
    const panel = document.createElement("div"); panel.id = SAVE_DIALOG_ID; panel.className = "save-format-dialog"; panel.setAttribute("role", "dialog"); panel.setAttribute("aria-modal", "true"); panel.setAttribute("aria-labelledby", "saveDialogTitle");
    ...
    const input = panel.querySelector("#saveFilename"); const formatSelect = panel.querySelector("#saveFormatSelect"); let closed = false;
    ...
    window.addEventListener("keydown", onGlobalKeydown, true); requestAnimationFrame(() => { input.focus(); input.select(); }); return panel;
}
