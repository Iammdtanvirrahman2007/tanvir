...
const STORAGE_KEY = "modelforge:workspace:v3";
...
function installMobileDock() {
    ...
    const dock = document.createElement("div"); dock.id = "mobileActionDock";
    ...
    dock.addEventListener("click", event => { const action = event.target.closest("[data-action]")?.dataset.action; if (!action) return; const click = id => document.getElementById(id)?.click(); const actions = { scene: () => click("mobileSceneBtn"), add: () => click("addMenuBtn"), select: () => click("selectBtn"), move: () => click("moveBtn"), rotate: () => click("rotateBtn"), scale: () => click("scaleBtn"), inspector: () => click("mobileInspectorBtn"), more: () => openMobileMore() }; actions[action]?.(); });
}

function openMobileMore() {
    let sheet = document.getElementById("mobileMoreSheet");
    if (sheet) { sheet.toggleAttribute("hidden"); return; }
    sheet = document.createElement("div"); sheet.id = "mobileMoreSheet"; sheet.className = "mobile-more-sheet";
    ...
    sheet.addEventListener("click", event => { if (event.target.closest("[data-close]")) return sheet.remove(); const id = event.target.closest("[data-click]")?.dataset.click; if (id) { document.getElementById(id)?.click(); sheet.remove(); } });
}
...
function installTouchSafety() {
    ...
    let lastTap = 0;
    viewport.addEventListener("pointerup", event => { if (event.pointerType !== "touch") return; const now = performance.now(); if (now - lastTap < 280) window.dispatchEvent(new CustomEvent("editor:double-tap", { detail: { x: event.clientX, y: event.clientY } })); lastTap = now; }, { passive: true });
    window.addEventListener("editor:double-tap", () => document.getElementById("frameBtn")?.click());
}

export function getQualityProfile() { return { performance: root.dataset.performance || "balanced", device: root.dataset.device || "desktop" }; }

export function setQualityMode(mode) {
    if (!["performance", "balanced", "high"].includes(mode) || !renderer) return false;
    root.dataset.performance = mode;
    installQuality({ performance: mode });
    window.dispatchEvent(new CustomEvent("editor:quality-change", { detail: mode }));
    saveWorkspace();
    return true;
}
...
function bindOrientation() { if (screen.orientation?.addEventListener) screen.orientation.addEventListener("change", handleResize); }

function saveWorkspace() {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ device: root.dataset.device, input: root.dataset.input, performance: root.dataset.performance, tool: document.querySelector("#bottomToolbar .tool-btn.active")?.id || "selectBtn", grid: document.getElementById("gridBtn")?.textContent || "Grid" })); } catch {}
}

function restoreWorkspace() {
    try { const data = JSON.parse(localStorage.getItem(STORAGE_KEY) || "null"); if (data?.tool) setTimeout(() => document.getElementById(data.tool)?.click(), 0); } catch {}
}
