...
export function setupSelection(renderer, camera, scene) {
    ...
    canvas.addEventListener("pointerdown", event => { const tc = getTransform(); if (event.button !== 0 || isDraggingTransform() || tc?.axis) return; pointerStart = { x: event.clientX, y: event.clientY, pointerId: event.pointerId, shift: event.shiftKey }; pointerMoved = false; }, { passive: true });
    canvas.addEventListener("pointermove", event => { if (!pointerStart || event.pointerId !== pointerStart.pointerId) return; const distance = Math.hypot(event.clientX - pointerStart.x, event.clientY - pointerStart.y); if (distance > CLICK_DRAG_THRESHOLD) pointerMoved = true; if (pointerStart.shift && event.pointerType !== "touch" && !boxDragging && distance > CLICK_DRAG_THRESHOLD) startBoxSelection(event, pointerStart); if (boxDragging) updateBox(event); }, { passive: true });
    canvas.addEventListener("pointerup", event => { if (event.pointerId !== pointerStart?.pointerId) return; if (boxDragging) { finishBoxSelection(event, camera, scene); resetPointerState(); return; } resetPointerState(); }, { passive: true });
...
function installGroupSelectionSync() { if (groupSyncInstalled) return; groupSyncInstalled = true; window.addEventListener("editor:group-selection-sync", event => { const detail = event.detail || {}; const objects = normalizeObjects(detail.objects || []); if (window.__modelForgeNodeEditMode === true) { if (detail.mode === "single" && objects[0]?.userData?.attachmentNode) selectObject(objects[0]); return; } if (detail.mode === "single" && objects[0]) selectObject(objects[0]); else if (detail.mode === "multiple") selectMultiple(objects); else clearSelection(); }); }
...
function isEditorOnlyHit(object) { let current = object; while (current) { if (current.userData?.editorOnly || current.name === "VoxelWorkspace") return true; current = current.parent; } return false; }
function startBoxSelection(event, start) { if (window.__modelForgeNodeEditMode === true) return; boxDragging = true; boxStart = { x: start.x, y: start.y }; selectionCanvas.setPointerCapture?.(event.pointerId); boxElement = document.createElement("div"); boxElement.className = "selection-box-overlay"; Object.assign(boxElement.style, { position: "fixed", zIndex: "70", pointerEvents: "none", border: "1px solid #d97706", background: "rgba(217,119,6,.10)", boxSizing: "border-box" }); document.body.appendChild(boxElement); updateBox(event); }
