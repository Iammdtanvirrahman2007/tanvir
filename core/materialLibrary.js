...
export function registerMaterial(name, material) { const key=normalize(name); if(!key||!material)return null; materials.set(key,serializeMaterial(name,material)); save(); dispatch("editor:material-library-change",{action:"register",name}); return materials.get(key); }
...
export function renameMaterial(oldName,newName) { const oldKey=normalize(oldName),newKey=normalize(newName); if(!materials.has(oldKey)||!newKey||(newKey!==oldKey&&materials.has(newKey)))return false; const data=materials.get(oldKey); data.name=newName.trim(); materials.delete(oldKey); materials.set(newKey,data); save(); dispatch("editor:material-library-change",{action:"rename",oldName,newName}); return true; }
...
export function applyMaterial(object,name) { if(!object)return false; const source=getMaterial(name); if(!source)return false; if(Array.isArray(object.material))object.material=object.material.map(()=>source.clone()); else object.material=source.clone(); const applied=Array.isArray(object.material)?object.material[0]:object.material; if(applied)applied.needsUpdate=true; object.userData.materialName=name; dispatch("editor:material-applied",{object,name}); return true; }
export function serializeMaterialState(material,name=material?.name||"") { if(!material)return null; return {name,color:material.color?.getHex?.()??0xffffff,metalness:material.metalness??0,roughness:material.roughness??1,opacity:material.opacity??1,transparent:!!material.transparent,depthWrite:material.depthWrite??true,wireframe:!!material.wireframe,flatShading:!!material.flatShading,side:material.side??THREE.FrontSide,emissive:material.emissive?.getHex?.()??0,emissiveIntensity:material.emissiveIntensity??1,materialPreset:material.userData?.materialPreset||"Custom",baseTextureData:material.userData?.baseTextureData||null,textureSlots:material.userData?.textureSlots||null,textureTransform:material.userData?.textureTransform||null}; }
export function deserializeMaterialState(data) { if(!data)return new THREE.MeshStandardMaterial({color:0xffffff,roughness:.65}); const material=new THREE.MeshStandardMaterial({color:data.color??0xffffff,metalness:data.metalness??0,roughness:data.roughness??1,opacity:data.opacity??1,transparent:data.transparent??false,depthWrite:data.depthWrite??((data.opacity??1)>=1),wireframe:data.wireframe??false,side:data.side??THREE.FrontSide,emissive:data.emissive??0,emissiveIntensity:data.emissiveIntensity??1}); material.name=data.name||""; material.flatShading=!!data.flatShading; material.userData.materialPreset=data.materialPreset||"Custom"; if(data.textureSlots)material.userData.textureSlots=data.textureSlots; if(data.textureTransform)material.userData.textureTransform=data.textureTransform; if(data.baseTextureData){material.userData.baseTextureData=data.baseTextureData;restoreTexture(material,data.baseTextureData);} return material; }
