...
export function loadScene(scene) {
    ...
    input.addEventListener("change", event => { const file = event.target.files?.[0]; if (file) openProjectFile(scene, file); input.value = ""; }, { once: true });
    input.click();
}
...
async function openGLTF(scene, file) {
    const { GLTFLoader } = await import("three/addons/loaders/GLTFLoader.js"); const loader = new GLTFLoader(); const buffer = await file.arrayBuffer();
    const result = await new Promise((resolve, reject) => loader.parse(buffer, "", resolve, reject)); return addImportedRoot(scene, result.scene, file, "glTF");
}

async function openLoader(scene, file, loaderName) {
    const module = await import(`three/addons/loaders/${loaderName}.js`); const Loader = module[loaderName]; if (!Loader) throw new Error(`${loaderName} unavailable`); const loader = new Loader();
    const source = loaderName === "STLLoader" || loaderName === "PLYLoader" ? await file.arrayBuffer() : await file.text(); const parsed = loader.parse(source);
    const root = parsed.isObject3D ? parsed : new THREE.Mesh(parsed, new THREE.MeshStandardMaterial({ color: 0xb7c0ca, roughness: 0.6 })); return addImportedRoot(scene, root, file, loaderName.replace("Loader", ""));
}

function addImportedRoot(scene, root, file, format) {
    clearObjects(scene); root.name = file.name.replace(/\.[^.]+$/, "") || "Imported Model"; root.userData.editorObject = true;
    root.traverse(object => { object.userData.editorObject = true; if (object.isMesh) { object.castShadow = true; object.receiveShadow = true; } });
...
function createMaterial(data) {
    ...
    if (data?.baseTextureData) {
        ...
        try {
            new THREE.TextureLoader().load(data.baseTextureData, texture => {
