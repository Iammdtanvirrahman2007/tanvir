...
function render() {
    ...
    if (!nodes.length) {
        ...
        editor.innerHTML = `<div class="rocket-node-hint">Nodes are stored in the rocket part's local coordinate system. Auto placement is only a starting point; fine-tune with the viewport gizmo.</div>`;
    } else if (activeId) {
...
function buildEditor(editor, node) {
    editor.innerHTML = `
        ...
        <div class="node-field-label">Position (local)</div><div class="node-vector" data-position></div>
