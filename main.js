...
import { groupSelected, ungroupSelected, setMultiSelection, getMultiSelection } from "./core/grouping.js";
...
function bindUI() {
    document.getElementById("sceneAddBtn")?.addEventListener("click", event => {
        event.stopPropagation();
        toggleAddMenu(event.currentTarget);
    });

    document.getElementById("addMenuBtn")?.addEventListener("click", event => toggleAddMenu(event.currentTarget));
    ...
    document.getElementById("gridBtn")?.addEventListener("click", () => {
        ...
        document.getElementById("gridBtn").classList.toggle("active", grid.visible);
        setStatus(grid.visible ? "Grid enabled" : "Grid hidden");
    });
    ...
    document.getElementById("snapBtn")?.addEventListener("click", () => {
        ...
        button.classList.toggle("snap-on", enabled);
    });

    document.getElementById("groupBtn")?.addEventListener("click", () => {
        const group = groupSelected(scene);
        if (group) {
            setStatus(`Grouped ${group.name}`);
            rebuildHierarchy();
        } else {
            setStatus("Select two or more objects with Ctrl-click");
        }
    });

    document.getElementById("ungroupBtn")?.addEventListener("click", () => {
        const selected = getSelected();
        const children = ungroupSelected(scene, selected);
        if (children.length) setStatus("Group ungrouped");
        else setStatus("Select a group first");
    });

    document.getElementById("addCollectionBtn")?.addEventListener("click", () => {
        setStatus("Collections are represented by Groups. Use Ctrl-click then Group.");
    });

    document.getElementById("sceneOptionsBtn")?.addEventListener("click", () => {
        setStatus(`${getObjects().length} editor objects`);
    });

    document.addEventListener("pointerdown", event => {
        const popover = document.getElementById("menuPopover");
        if (!popover || popover.hidden) return;
        if (!popover.contains(event.target) && !event.target.closest("#sceneAddBtn,#addMenuBtn")) closeMenu();
    });
}
...
function bindEditorEvents() {
    ...
    window.addEventListener("editor:frame", event => {
        const fps = event.detail?.fps ?? getFPS();
        const el = document.getElementById("fpsCounter");
        if (el) el.textContent = fps;
    });

    window.addEventListener("editor:transform-mode", event => {
        state.transformMode = event.detail;
        syncToolButtons();
    });

    window.addEventListener("editor:multiselect-change", event => {
        const selection = event.detail || [];
        if (selection.length > 1) {
            setStatus(`${selection.length} objects selected`);
        }
    });
...
function bindKeyboard() {
    window.addEventListener("keydown", event => {
        ...
        if (event.ctrlKey && key === "z") {
            ...
            if (undo()) setStatus("Undo");
            return;
        }
        if (event.ctrlKey && key === "y") {
            event.preventDefault();
            if (redo()) setStatus("Redo");
            return;
        }
...
function toggleAddMenu(anchor) {
    ...
    popover.innerHTML = `
        <button data-add="cube">Cube <span class="shortcut">Shift+C</span></button>
    ...
    popover.querySelectorAll("[data-add]").forEach(button => {
