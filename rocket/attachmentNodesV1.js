...
export function initAttachmentNodeFoundation(scene) {
    sceneRef = scene || null;
    activeNodeId = null;
}
...
function dispatchChange(node) {
    window.dispatchEvent(new CustomEvent("editor:rocket-node-change", { detail: node || null }));
}

function dispatchSelection(node = null) {
    window.dispatchEvent(new CustomEvent("editor:attachment-node-selected", { detail: node || null }));
}
