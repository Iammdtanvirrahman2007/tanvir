(() => {
  const register = () => {
    ...
    import('./rocket/nodeVectorRenderer.js?v=20260812-node-vector-render-4').then(module => {
      const start = () => { try { module.initNodeVectorRenderer(); } catch (error) { console.warn('ModelForge node vector renderer failed:', error); } };
      requestAnimationFrame(() => requestAnimationFrame(start));
    }).catch(error => console.warn('ModelForge node vector renderer load failed:', error));

    import('./core/voxel/editor.js?v=20260818-voxel-ui-3').then(async ({ initVoxelEditor }) => {
      ...
      const start = () => {
        ...
        if (!scene || !renderer || !camera) { requestAnimationFrame(start); return; }
    ...
    import('./core/voxel/previewController.js?v=20260818-greedy-2').then(module => {
    ...
    import('./core/voxel/voxel2Controller.js?v=20260818-v2-1').then(module => {
      try { module.initVoxel2Controller(); } catch (error) { console.warn('ModelForge voxel 2.0 controller failed:', error); }
    }).catch(error => console.warn('ModelForge voxel 2.0 controller load failed:', error));

    import('./core/productionAssetUI.js?v=20260818-production-asset-2').then(async module => {
      try {
        const { scene } = await import('./core/scene.js?v=20260811-runtime-fix');
        if (scene) module.initProductionAssetUI(scene);
        else requestAnimationFrame(() => import('./core/scene.js?v=20260811-runtime-fix').then(({ scene }) => scene && module.initProductionAssetUI(scene)).catch(() => {}));
    ...
    Promise.all([
      ...
      ai.initAIAssistant({ getSceneContext: () => {
        const objects = objectManager.getObjects();
        const selected = selection.getSelected();
        return { transformSpace: transform.getTransformSpace(), selected: toTransform(selected), objectCount: objects.length, objects: objects.map(toTransform) };
      }});
